<template>
    ...
    <el-form-item label="" style="margin-bottom: 0.1; margin-right: 10px;">
      <el-input v-model="params.title" placeholder="输入要搜索的竞赛名称" />
    </el-form-item>
    <el-form-item >
  ...
  <el-table height="526px" :data="examList">
    ...
    <el-table-column prop="createName" width="120px" label="创建用户" />
...
function onAddExam() {
  //跳转到新的页面上   
  router.push("/jcode/layout/updateExam?type=add")
}

async function onEdit(examId) {
  // await getExamDetailService(examId)
  router.push(`/jcode/layout/updateExam?type=edit&examId=${examId}`)
